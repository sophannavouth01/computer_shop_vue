<template>
  <div class="grid justify-items-end  pr-5  mb-3">
    <div class=" ">
      <router-link to="add-brand"><i class="fa-solid fa-square-plus text-[25px]"></i></router-link>
    </div>
      ...
      <tbody>
        ...
        class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-blue-400">
            ...
            <td class="px-6 py-4">
              ...
              <button @click="this.$router.push(`edit-product/${b.id}`)" >
...
export default {
  ...
  methods: {
    fetchDataFromApi() {
      const apiUrl = 'http://127.0.0.1:8000/api/brands'
      axios.get(apiUrl)
        .then((response) => {
          console.log(response.data.brands)
          this.ProductData = response.data.brands
        })
        .catch((error) => {
    ...
    getImageUrl(relativePath) {
      // Assuming the base URL for images
